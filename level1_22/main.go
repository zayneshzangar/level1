...
func NewCalculator(a, b int64) (*Calculator, error) {
    ...
    useBig := aBig.Cmp(maxInt64) > 0 || bBig.Cmp(maxInt64) > 0 || sum.Cmp(maxInt64) > 0 || product.Cmp(maxInt64) > 0
...
func main() {
    testCases := []struct {
        ...
        {1 << 21, 1 << 22, "Small numbers (within int64)"},
        {1 << 40, 1 << 40, "Large numbers (beyond int64)"},
        {1 << 62, 1 << 62, "Near overflow (sum exceeds int64)"},
    }
